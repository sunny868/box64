...
GO("libSDL3.so.0", sdl3)
GO("libSDL3_ttf.so.0", sdl3ttf)
...
GO("libayatana-indicator3.so.7", ayatanaindicator3)
GO("libayatana-ido3-0.4.so.0", ayatanaido3)
...
GO("libharfbuzz-subset.so.0", harfbuzzsubset)
GO("libbz2.so.1", bz2)
...
GO("libomp.so.5", libomp)
GO("libcap.so.2", cap)
GO("libpcap.so.0.8", pcap)
GO("libpcap.so.1", pcap)
GO("libpcap.so.1.10.6", pcap)
...
GO("libicuuc.so.68", icuuc68)
GO("libicui18n.so.68", icui18n68)
